...
public class CoverageServiceRepository {
    ...
    static final String SELECT_DELTA =
            "SELECT cov1.bene_coverage_period_id, cov1.beneficiary_id, :type as entryType, CURRENT_TIMESTAMP as created " +
                    "FROM coverage cov1 " +
            "LEFT JOIN coverage cov2 ON cov1.ID = cov2.ID " +
            "WHERE cov2.ID IS NULL";
             /*
            "SELECT cov1.bene_coverage_period_id, cov1.beneficiary_id, :type as entryType, CURRENT_TIMESTAMP as created from coverage cov1 " +
                    "WHERE bene_coverage_search_event_id = :search1 " +
            "LEFT JOIN coverage cov2 WHERE bene_coverage_search_event_id = :search2 on cov1.id = cov2.id";
            */
