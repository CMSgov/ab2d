...
SELECT setval('public.user_account_seq', COALESCE((SELECT MAX(id) FROM public.user_account), 1));
...
SELECT setval('public.job_seq', COALESCE((SELECT MAX(id) FROM public.job), 1));
...
SELECT setval('public.job_output_seq', COALESCE((SELECT MAX(id) FROM public.job_output), 1));
...
SELECT setval('public.event_bene_coverage_search_status_change_seq', COALESCE((SELECT MAX(id) FROM public.event_bene_coverage_search_status_change), 1));
...
SELECT setval('public.bene_coverage_period_seq', COALESCE((SELECT MAX(id) FROM public.bene_coverage_period), 1));
...
SELECT setval('coverage_search_seq', COALESCE((SELECT MAX(id) FROM public.coverage_search), 1));
