...
public interface CoverageDriver {
    ...
    /**
     * Check database for all {@link gov.cms.ab2d.coverage.model.CoveragePeriod} that are missing information completely
     ...
     * @throws InterruptedException if thread is interrupted trying to obtain lock
     */
    boolean isCoverageAvailable(Job job, ContractWorker contract) throws InterruptedException;
     ...
     */
    int numberOfBeneficiariesToProcess(Job job, ContractWorker contract);
     ...
     */
    CoveragePagingResult pageCoverage(Job job, ContractWorker contract);
