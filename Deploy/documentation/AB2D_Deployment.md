...
1. [Create AWS credentials for signing requests for getting public S3 files](#create-aws-credentials-for-signing-requests-for-getting-public-s3-files)
...
## Create AWS credentials for signing requests for getting public S3 files

1. Open Chrome

1. Log on to the management AWS account

1. Select **IAM**

1. Create an "Ab2dS3Signing" group
...
1. Create an "ab2d-s3-signing" user
   ...
   1. Check the checkbox beside **Ab2dS3Signing**

   1. Select **Next: Tags**

   1. Select **Next: Review**

   1. Note that this user has no permissions
...
1. Note that the following can be found in the "credentials.csv" file that you downloaded
   ...
   *Format:*
   ...
   - **Password:** {blank because console access was not setup}

   - **Access key ID:** {ab2d-s3-signing access key}

   - **Secret access key:** {ab2d-s3-signing secret access key}

   - **Console login link:** https://653916833532.signin.aws.amazon.com/console

1. Save these credentials to 1Password

   - **ab2d-s3-signing AWS access key id:** {ab2d-s3-signing access key}

   - **ab2d-s3-signing AWS secret access key:** {ab2d-s3-signing secret access key}

1. Configure AWS profile for "ab2d-s3-signing"

   ```ShellSession
   $ aws configure --profile=ab2d-s3-signing
   ```
