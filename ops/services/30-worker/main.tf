...
resource "aws_ecs_task_definition" "worker" {
  ...
  container_definitions = nonsensitive(jsonencode([{
    ...
    secrets : [
      ...
      { name : "AB2D_BFD_KEYSTORE_PRIVATE_KEY", value : "/ab2d/${local.env}/api/sensitive/tls_private_key" },
      { name : "AB2D_BFD_KEYSTORE_CERTIFICATE", value : "/ab2d/${local.env}/api/nonsensitive/tls_public_cert" },
