...
BASIC_AUTH=$(aws secretsmanager get-secret-value --secret-id ab2d/ab2d-sbx-sandbox/jenkins-verify-basic-auth --query SecretString --output text)
...
while [ $STATE != 401 ]; do
    ...
    STATE=$(curl -sLk -w "%{http_code}" "https://sbx.ab2d.cms.gov" -o /dev/null)
    echo $STATE
...
curl -k -s --head --location --request GET 'https://sbx.ab2d.cms.gov/api/v2/fhir/Patient/$export?_type=ExplanationOfBenefit&_since=2020-02-13T00:00:00.000-05:00&_outputFormat=application%2Ffhir%2Bndjson' \
