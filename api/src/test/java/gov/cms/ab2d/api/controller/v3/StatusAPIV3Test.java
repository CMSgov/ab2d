...
class StatusAPIV3Test {
  ...
  void testStatus() throws Exception {
    ...
    this.mockMvc.perform(
      get("https://localhost:8443/api/v3/fhir/Patient/$export").contentType(FHIR_NDJSON_CONTENT_TYPE)
    ...
    ResultActions resultActions = this.mockMvc.perform(
      get(String.format("https://localhost:8443/api/v3/fhir/Job/%s/$status", jobUuid)).contentType(FHIR_NDJSON_CONTENT_TYPE)
        .header("Authorization", "Bearer " + token));
 ...
 // @Test
  void testDelete() throws Exception {
    ...
    String jobUuid = jobClientMock.pickAJob();
    ResultActions resultActions = this.mockMvc.perform(
      delete(String.format("https://localhost:8443/api/v3/fhir/Job/%s/$status", jobUuid)).contentType(FHIR_NDJSON_CONTENT_TYPE)
        .header("Authorization", "Bearer " + token));
