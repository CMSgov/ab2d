 ...
 *
 * Inspired from https://github.com/HedvigInsurance/product-pricing/blob/master/src/main/java/com/hedvig/common/LogstashRequestHeaderFilter.java
 */
public class LogstashHeaderFilter implements HeaderFilter {
    ...
    public boolean includeHeader(String headerName, String headerValue) {
        if (includes.isEmpty() && excludes.isEmpty()) {
            ...
            throw new IllegalStateException(String.format("Both includes (%s) and excludes (%s) cannot be configured at the same time. " +
                    "Only one or the other, or neither can be configured.", includes, excludes));
        }
    }
